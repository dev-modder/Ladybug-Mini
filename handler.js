 ...
 * Ladybug Bot V5 — Updated by Dev-Ntando
 *
 * V5 additions:
 *   • handleAntiSpam   — rate-limit spammers, warn & kick
 *   • handleWordFilter — delete messages containing banned words
 *   • handlePollMessage — acknowledge & log WhatsApp native polls
 *   • logCommand integration — every executed command is persisted
 */
...
const handleMessage = async (sock, msg) => {
  try {
    ...
    // ── V5: Log every command execution ──
    database.logCommand(commandName, sender, from);
...
// ═══════════════════════════════════════════════════════════════
// 🚫 V5 — Anti-Spam Handler
// Detects users sending too many messages in a short window.
// Config fields used:
//   groupSettings.antispam      — true/false
//   groupSettings.antispamLimit — messages per 5 s before action (default 5)
//   groupSettings.antispamAction— 'warn' | 'kick' (default 'warn')
// ═══════════════════════════════════════════════════════════════
const handleAntiSpam = async (sock, msg, groupMetadata) => {
  try {
    const from   = msg.key.remoteJid;
    const sender = msg.key.participant || msg.key.remoteJid;

    if (msg.key.fromMe) return; // never flag the bot itself

    const groupSettings = database.getGroupSettings(from);
    if (!groupSettings.antispam) return;

    const limit  = groupSettings.antispamLimit  || 5;
    const action = (groupSettings.antispamAction || 'warn').toLowerCase();

    // Record this message and get the rolling window count
    const record = database.recordSpamMessage(from, sender);
    const count  = record.timestamps.length;

    if (count < limit) return; // under the threshold — all good
    ...
    if (action === 'kick' && botIsAdmin) {
      database.clearSpamRecord(from, sender);
      try {
        await sock.groupParticipantsUpdate(from, [sender], 'remove');
        await sock.sendMessage(from, {
          text: `🚫 *Anti-Spam* | @${sender.split('@')[0]} was removed for spamming (${count} messages in 5s).`,
          mentions: [sender]
        });
      } catch (e) {
      ...
      // Warn once per spam burst
      if (record.warned) return;
      database.markSpamWarned(from, sender);

      const warning = database.addWarning(from, sender, `Spam — ${count} messages in 5s`);

      let warningText = `⚠️ *Anti-Spam Warning* | @${sender.split('@')[0]}\n`;
      warningText    += `Slow down! You sent ${count} messages in 5 seconds.\n`;
      warningText    += `Warnings: ${warning.count}/3`;
      ...
      // Auto-kick on 3rd warning if bot is admin
      if (warning.count >= 3 && botIsAdmin) {
        database.clearSpamRecord(from, sender);
        database.clearWarnings(from, sender);
        try {
          await sock.groupParticipantsUpdate(from, [sender], 'remove');
          await sock.sendMessage(from, {
            text: `🚫 @${sender.split('@')[0]} was removed after 3 spam warnings.`,
            mentions: [sender]
          });
        } catch (e) {
          console.error('[AntiSpam auto-kick error]', e.message);
        }
      }
    }
  } catch (error) {
...
// ═══════════════════════════════════════════════════════════════
// 🤬 V5 — Word Filter Handler
// Deletes messages that contain banned words.
// Config fields used:
//   groupSettings.wordfilter      — true/false
//   groupSettings.wordfilterAction— 'delete' | 'warn' | 'kick' (default 'delete')
// Banned words are managed via database.addBannedWord / removeBannedWord.
// ═══════════════════════════════════════════════════════════════
const handleWordFilter = async (sock, msg, groupMetadata) => {
  try {
    const from   = msg.key.remoteJid;
    const sender = msg.key.participant || msg.key.remoteJid;
    ...
    const groupSettings = database.getGroupSettings(from);
    if (!groupSettings.wordfilter) return;

    // Build message body from all text-bearing message types
    ...
    const { found, word } = database.checkWordFilter(body, from);
    if (!found) return;
    ...
    const botIsAdmin = await isBotAdmin(sock, from, groupMetadata);
    const action     = (groupSettings.wordfilterAction || 'delete').toLowerCase();
    ...
    if (action === 'kick' && botIsAdmin) {
      try {
        await sock.groupParticipantsUpdate(from, [sender], 'remove');
        await sock.sendMessage(from, {
          text: `🚫 *Word Filter* | @${sender.split('@')[0]} was removed for using a banned word.`,
          mentions: [sender]
        });
      } catch (e) {
        console.error('[WordFilter kick error]', e.message);
      }
    } else if (action === 'warn') {
      const warning = database.addWarning(from, sender, `Banned word: "${word}"`);
      await sock.sendMessage(from, {
        text: `⚠️ *Word Filter* | @${sender.split('@')[0]}, that word is not allowed here!\nWarnings: ${warning.count}/3`,
        mentions: [sender]
      });
      ...
      if (warning.count >= 3 && botIsAdmin) {
        ...
        try {
          await sock.groupParticipantsUpdate(from, [sender], 'remove');
          await sock.sendMessage(from, {
            text: `🚫 @${sender.split('@')[0]} was removed after 3 word-filter warnings.`,
            mentions: [sender]
          });
        } catch (e) {
          console.error('[WordFilter auto-kick error]', e.message);
        }
      }
    } else {
      // Default: silent delete — already done above, optionally notify
      await sock.sendMessage(from, {
        text: `🚫 *Word Filter* | A message was removed for containing a banned word.`
      });
    }
  } catch (error) {
    console.error('[WordFilter Error]', error.message);
  }
};


// ═══════════════════════════════════════════════════════════════
// 📊 V5 — Poll Message Handler
// Acknowledge and log WhatsApp native poll creation & updates.
// ═══════════════════════════════════════════════════════════════
const handlePollMessage = async (sock, msg) => {
  try {
    const from   = msg.key.remoteJid;
    const sender = msg.key.participant || msg.key.remoteJid;
    ...
    if (msg.message?.pollCreationMessage) {
      ...
      const name = poll.name || 'Unnamed Poll';
      const opts = (poll.options || []).map((o, i) => `  ${i + 1}. ${o.optionName}`).join('\n');

      // Log poll creation as a command entry
      database.logCommand('poll:create', sender, from);

      console.log(`[Poll Created] "${name}" in ${from} by ${sender.split('@')[0]}`);

      // Optional: react to the poll to acknowledge it
      try {
        await sock.sendMessage(from, {
          react: { text: '📊', key: msg.key }
        });
    ...
    if (msg.message?.pollUpdateMessage) {
      database.logCommand('poll:vote', sender, from);
      console.log(`[Poll Vote] from ${sender.split('@')[0]} in ${from}`);
    }
  } catch (error) {
    console.error('[PollHandler Error]', error.message);
  }
};
