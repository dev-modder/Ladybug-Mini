 ...
 * Ahegao Command - Get random ahegao anime images
 */
...
module.exports = {
  ...
  aliases: ['ahegaonsfw'],
  category: 'anime',
  desc: 'Get random ahegao NSFW anime images',
  usage: 'ahegao',
  execute: async (sock, msg, args, extra) => {
    try {
      ...
      const tempImagePath = path.join(tempDir, `ahegao_${timestamp}.${extension}`);
      ...
      console.error('Error in ahegao command:', error);
      ...
      if (error.response?.status === 404) {
        ...
        await extra.reply(`❌ Failed to fetch ahegao image: ${error.message}`);
      }
    }
  }
};
