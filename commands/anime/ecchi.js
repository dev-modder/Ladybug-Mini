 ...
 * Ecchi Command - Get random ecchi anime images
 */
...
module.exports = {
  ...
  aliases: ['ecchinsfw'],
  category: 'anime',
  desc: 'Get random ecchi NSFW anime images',
  usage: 'ecchi',
  execute: async (sock, msg, args, extra) => {
    try {
      ...
      const tempImagePath = path.join(tempDir, `ecchi_${timestamp}.${extension}`);
      ...
      console.error('Error in ecchi command:', error);
      ...
      if (error.response?.status === 404) {
        ...
        await extra.reply(`❌ Failed to fetch ecchi image: ${error.message}`);
      }
    }
  }
};
