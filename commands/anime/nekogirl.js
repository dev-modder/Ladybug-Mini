 ...
 * Nekogirl Command - Get random nekogirl anime images
 */
...
const BASE = 'https://api.princetechn.com/api/anime/nekogirl';
const API_KEY = 'prince';

module.exports = {
  name: 'nekogirl',
  aliases: ['catgirl', 'nekogirlsfw'],
  category: 'anime',
  desc: 'Get random nekogirl SFW anime images',
  usage: 'nekogirl',
  execute: async (sock, msg, args, extra) => {
    try {
      ...
      const tempImagePath = path.join(tempDir, `nekogirl_${timestamp}.${extension}`);
      ...
      console.error('Error in nekogirl command:', error);
      ...
      if (error.response?.status === 404) {
        ...
        await extra.reply(`❌ Failed to fetch nekogirl image: ${error.message}`);
      }
    }
  }
};
