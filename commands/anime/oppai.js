 ...
 * Oppai Command - Get random oppai anime images
 */
...
const BASE = 'https://api.princetechn.com/api/anime/oppai';
...
module.exports = {
  ...
  aliases: ['oppainsfw'],
  category: 'anime',
  desc: 'Get random oppai NSFW anime images',
  usage: 'oppai',
  execute: async (sock, msg, args, extra) => {
    try {
      ...
      const tempImagePath = path.join(tempDir, `oppai_${timestamp}.${extension}`);
      ...
      console.error('Error in oppai command:', error);
      ...
      if (error.response?.status === 404) {
        ...
        await extra.reply(`❌ Failed to fetch oppai image: ${error.message}`);
      }
    }
  }
};
