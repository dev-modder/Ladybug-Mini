...
const STICKER_LIMIT = 950 * 1024;          // 950 KB
...
async function convertBufferToStickerWebp(inputBuffer, isAnimated, cropSquare) {
  ...
  const base = `igs_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`;
  ...
  try {
    ...
    // Progressive re-compression until under STICKER_LIMIT
    let attempt = 0;
    const maxAttempts = 8;
    while (webpBuffer.length > STICKER_LIMIT && attempt < maxAttempts) {
      ...
      try {
        ...
        if (fs.existsSync(outN)) {
          webpBuffer = fs.readFileSync(outN);
        }
      } catch { /* keep trying */ }
    }
    ...
    // Absolute safety: force tiny sizes if still over limit
      ...
      for (const size of [256, 200, 180, 160, 128]) {
        ...
        try {
          ...
          if (fs.existsSync(outM)) {
            ...
            if (candidate.length <= STICKER_LIMIT) {
    ...
    for (const size of [256, 200, 180, 160]) {
      ...
      try {
        ...
        if (fs.existsSync(tempOutput)) {
          ...
          if (buf.length <= STICKER_LIMIT) {
            return await attachExifAndSave(buf);
          }
        }
      } catch { /* try next size */ }
    }
...
async function ultraCompressSticker(inputBuffer, isVideo, cropSquare, itemIndex) {
  ...
  try {
    ...
    await execAsync(cmd);
    if (fs.existsSync(tempOutput)) {
      return await attachExifAndSave(fs.readFileSync(tempOutput));
    }
    return null;
  } catch {
    return null;
  ...
  try {
    ...
    for (let i = 0; i < items.length; i++) {
      try {
        ...
        // Fallback compressions if still over limit
        ...
        try {
          ...
          console.error(`[igs] Failed to send sticker ${i + 1}:`, sendErr.message);
          failCount++;
        }
        ...
        console.error(`[igs] Error processing item ${i + 1}:`, itemErr.message);
        failCount++;
      }
    }
